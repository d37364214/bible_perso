...
const elements = {
    ...
    manageBibleSelect, renameBibleBtn, deleteBibleBtn, versionSelect, modifiedVersesToggle
};
...
const CACHE_KEY = name => `cache_${encodeURIComponent(name)}`;
    ...
    if (deleteBibleBtn) deleteBibleBtn.addEventListener('click', () => {
        ...
        if (versionToDelete && versionToDelete !== 'Version Originale') {
            ...
            if (confirmed) {
                ...
                updateVersionSelectors();
                // Tente de revenir à la dernière version active, sinon la version originale
